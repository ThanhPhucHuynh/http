...
func header(inputHeaders http.Header) http.Header {
	h := http.Header{}
	for key, values := range defaultHeaders {
		h[key] = values
	}
	for key, values := range inputHeaders {
		h[key] = values
	}
	return h
}
...
func (hhc *Client) AddPlugin(p avhttp.Plugin) {
